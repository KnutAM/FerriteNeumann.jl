...
#= Alternative method which in one way doesn't include the "weak form" per say, but
# I'm not sure if it brings any advantages. =#
function calculate_neumann_contribution2!(fe::Vector, face::FaceIterator, fv::FaceValues, time, fun)
    fill!(fe, 0)
    reinit!(fv, face)
    for q_point in 1:getnquadpoints(fv)
        x = spatial_coordinate(fv, q_point, getcoordinates(face))
...
function neumann_f_example(fv, q_point, i, x, time)
    δu = shape_value(fv, q_point, i)
    b = my_b_fun(x, time, getnormal(fv, q_point))
    dΓ = getdetJdV(fv, q_point)
    return (δu ⋅ b) * dΓ
end 
