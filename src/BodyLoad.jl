"""
    BodyLoad(field_name::Symbol, cv_info::Union{FaceValues,QuadratureRule,Int}, cellset::Set{Int}, f)
    BodyLoad(field_name::Symbol, cv_info::Union{FaceValues,QuadratureRule,Int}, f)
...
BodyLoad(fieldname::Symbol, fv_info, f) = BodyLoad(fieldname, fv_info, nothing, f)
...
function add_bodyload!(bodyloads::Vector, bodyload::BodyLoad, dh::MixedDofHandler)
    ...
    for fh in dh.fieldhandlers
        ...
        if !isempty(cellset) && hasfieldname(fh, bodyload.fieldname)
            contribution = true
            cell = getcells(dh.grid, first(cellset))
